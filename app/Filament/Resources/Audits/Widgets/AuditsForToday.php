...
class AuditsForToday extends BaseWidget
{
    ...
    protected static ?string $heading = 'Auditorías Programadas para Hoy';
        ...
        return $table
            ->query(
                ActivityExecution::query()
                    ->with('activity')
                    ->whereHas('activity', fn ($query) => $query->where('tipo', 'auditoria'))
            ...
            ->actions([
                ...
                Action::make('iniciar')
                    ...
                    ->modalHeading('Ejecutar Auditoría')
                    ->modalSubmitActionLabel('Guardar')
                    ->action(function (ActivityExecution $record, array $data) {
                        \Illuminate\Support\Facades\Log::info('Inicio de acción guardar auditoría', ['record_id' => $record->id, 'data' => $data]);
                        
                        $evidenciaLocalPaths = $data['evidencia'] ?? [];
                        // Normalizar a array si es un string (aunque multiple() debería devolver array)
                        ...
                        if (!empty($evidenciaLocalPaths)) {
                            ...
                            foreach ($evidenciaLocalPaths as $localPath) {
                                \Illuminate\Support\Facades\Log::info('Procesando archivo local de auditoría', ['path' => $localPath]);
                                ...
                                if ($localDisk->exists($localPath)) {
                                    ...
                                    try {
                                        ...
                                        if ($googleDisk->exists($targetPath)) {
                                            ...
                                            \Illuminate\Support\Facades\Log::info('Archivo de auditoría subido exitosamente a Drive', ['drive_path' => $targetPath]);
                                        } else {
                                            \Illuminate\Support\Facades\Log::warning('El archivo de auditoría no parece existir en Drive tras la subida', ['drive_path' => $targetPath]);
                                        }
                                    } catch (\Exception $e) {
                                        \Illuminate\Support\Facades\Log::error("Error moviendo archivo de auditoría {$localPath}: " . $e->getMessage());
                                    }
                                } else {
                                    \Illuminate\Support\Facades\Log::warning('Archivo local de auditoría no encontrado', ['path' => $localPath]);
                                }
                            }
                        } else {
                            \Illuminate\Support\Facades\Log::info('No hay evidencias de auditoría para subir (evidenciaLocalPaths vacío)');
                        }
                        ...
                        \Filament\Notifications\Notification::make()
                            ->title('Auditoría Ejecutada')
                            ->success()
                            ->send();
                        \Illuminate\Support\Facades\Log::info('--- FIN GUARDADO AUDITS FOR TODAY ---');
                    }),
