                ...
                Action::make('executions')
                    ...
                    ->form(function ($record) {
                        ...
                        $getFields = function ($execution, $prefix = '') {
                            ...
                            // Campos dinámicos de Promociones
                            $fields[] = Section::make('Resultados de Promoción')
                                ->schema([
                                    Grid::make(2)
                                        ->schema([
                                            TextInput::make($fieldName('data.cantidad_beneficiarios'))
                                                ->label('Beneficiarios Alcanzados')
                                                ->numeric()
                                                ->default($getData('cantidad_beneficiarios')),
                                            Textarea::make($fieldName('data.material_entregado'))
                                                ->label('Material Entregado')
                                                ->default($getData('material_entregado'))
                                                ->rows(2),
                                            Textarea::make($fieldName('data.observaciones_impacto'))
                                                ->label('Observaciones de Impacto')
                                                ->default($getData('observaciones_impacto'))
                                                ->rows(2)
                                                ->columnSpanFull(),
                                        ]),
                                ]);
