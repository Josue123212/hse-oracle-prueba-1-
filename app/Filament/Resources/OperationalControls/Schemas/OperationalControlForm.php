        ...
        return $schema
            ->components([
                ...
                TextInput::make('veces_al_anio')
                    ...
                    ->formatStateUsing(fn ($state, $record) => $state ?? $record?->activity?->veces_al_anio),

                TextInput::make('ejecuciones_realizadas')
                    ->label('Ejecuciones Realizadas')
                    ->numeric()
                    ->readOnly()
                    ->default(fn ($record) => $record?->activity?->ejecuciones_realizadas ?? 0)
                    ->formatStateUsing(fn ($state, $record) => $state ?? $record?->activity?->ejecuciones_realizadas),
